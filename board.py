...
class Board:
    ...
    def possible_to_put_stone_at(self, _turn, _pos):
        _possible = False
        if self.board[_pos[X]][_pos[Y]] == Board.BLANK:
            for _dir in directions:
                if 0 < self._count_reversible_number_in_direction(_turn, _pos, _dir):
                    _possible = True
                    break
        return _possible

    def count_reversible_number(self, _turn, _pos):
        _reverse_num = 0
        for _dir in directions:
            _reverse_num += self._count_reversible_number_in_direction(_turn, _pos, _dir)
        return _reverse_num

    def _count_reversible_number_in_direction(self, _turn, _pos, _dir):
        _opponent_turn = Stone.reverse(_turn)
        _reverse_num = 0
        _cur_pos = (_pos[X] + _dir[X], _pos[Y] + _dir[Y])
        while self.board[_cur_pos[X]][_cur_pos[Y]] == _opponent_turn:
            _cur_pos = (_cur_pos[X] + _dir[X], _cur_pos[Y] + _dir[Y])
            _reverse_num += 1
        if self.board[_cur_pos[X]][_cur_pos[Y]] != _turn:
            _reverse_num = 0
        return _reverse_num

    def reverse_stones_from(self, _pos):
        _turn = self.board[_pos[X]][_pos[Y]]
        if _turn == Stone.BLACK or _turn == Stone.WHITE:
            for _dir in directions:
    ...
    def _reverse_stones_in_direction(self, _turn, _pos, _dir):
        ...
        _cur_pos = (_pos[X] + _dir[X], _pos[Y] + _dir[Y])
        while self.board[_cur_pos[X]][_cur_pos[Y]] == _opponent_turn:
            self.board[_cur_pos[X]][_cur_pos[Y]] = _turn
            _cur_pos = (_cur_pos[X] + _dir[X], _cur_pos[Y] + _dir[Y])
