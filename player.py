...
class ComputerPlayer(Player):
    ...
    def _show_stat(self, elapsed_time, pos, evaluation, evaluated_num):
        ...
        print(
            "Put: {} Eval: {:.2f} (num:{} time:{:.2f}s tpe:{:.2f}us)".format(
                self._pos2str(pos),
                evaluation,
                evaluated_num,
                elapsed_time,
...
class MinimaxPlayer(ComputerPlayer):
    def __init__(
        self, name, my_stone, middle_evaluator, final_evaluator, max_search_level=5
    ):
        super().__init__(
            name, my_stone, Minimax(my_stone, middle_evaluator, max_search_level)
        )
    ...
    def next_move(self, a_board):
        if a_board.count_blank() < self.max_search_level + 8:
            self.selector = Minimax(
                self.my_stone, self.final_evaluator, a_board.count_blank()
            )
...
class MinimaxThreadPlayer(ComputerPlayer):
    def __init__(
        ...
        super().__init__(
            name,
            my_stone,
            Minimax_Threaded(my_stone, middle_evaluator, max_search_level, num_thread),
        )
        self.final_evaluator = final_evaluator
        self.max_search_level = max_search_level
    ...
    def next_move(self, a_board):
        if a_board.count_blank() < self.max_search_level + 8:
            self.selector = Minimax_Threaded(
                self.my_stone,
