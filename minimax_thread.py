...
class Minimax_Sub:
    ...
    def _minimax(self, a_board, a_stone, level):
        level += 1
        _pos_list = get_positions_to_put_stone(a_board, a_stone)
        return self.do_search(a_board, a_stone, level, _pos_list)

    def do_search(self, a_board, a_stone, level, pos_list):
        if 0 < len(pos_list):
            _eval_list = {}
            for _p in pos_list:
                b = a_board.copy()
                ...
                if self.max_level <= level:
                    ...
                    _eval_list[_p] = self._minimax(b, stone.reverse(a_stone), level)[1]
